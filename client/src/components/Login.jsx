...
const Login = () => {
  ...
  const load = async (e) => {
    ...
    try {
      ...
      message.current.show({ severity: "success", summary: 'הצלחה', detail: 'ברוך הבא!', life: 1500 })
      setTimeout(() => {
        setVisible(false)
        navigate("/")
      }, 1000);
    } catch (error) {
      message.current?.show({ severity: "error", summary: 'שגיאה', detail: 'שם משתמש או סיסמה לא נכונים', life: 3000 })
    }
  ...
  return (
    <>
      ...
      <Dialog className='p-9 card ' visible={visible} onHide={() => { if (!visible) return; setVisible(false); navigate("/") }} style={{ border: "solid 5px var(--indigo-300)" }}>
        <form onSubmit={load}	>
          <h1 className="flex justify-content-center w-full 	" >התחברות</h1>
          <div className="card flex justify-content-center" style={{ display: "flex", flexDirection: "column", gap: "20px", alignItems: "center" }}>
            ...
            <div className="flex align-items-center gap-2">
              <Button label="התחבר" icon="pi pi-check" loading={loading} disabled={disabled} autoFocus className="p-3 w-full  border-1 border-white-alpha-30" />
              <Button label="ביטול" onClick={() => { setVisible(false); navigate("/") }} outlined className="p-3 w-full border-1 border-white-alpha-30 "></Button>
            </div>
          </div>
        </form>
